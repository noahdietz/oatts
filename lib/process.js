...
function processTransactions(api, parentOp, parentPath, topLevel, options) {
    ...
    var headers = processHeaders(parentOp, parentPath, topLevel, options)
 ...
 * @return {object}
 */
function processHeaders(op, path, top, options) {
    var headers = {};
    ...
    if (options.customValues) {
        ...
        if (options.customValues[path.path]) {
            ...
            if (options.customValues[path.path][op.method] && options.customValues[path.path][op.method].headers) {
    ...
    if (options.consumes) { // a specific content-type was targeted
        if (op.definitionFullyResolved.consumes) {
            ...
            } //else { not in the operation defined consumes, so must be excluded }
