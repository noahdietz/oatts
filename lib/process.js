...
function process(api, options) {
    var processedSpec = {
        ...
        'scheme': (options.scheme !== undefined ? options.scheme : (api.schemes !== undefined ? api.schemes[0] : 'http')),
...
function processTransactions(api, parentOp, parentPath, topLevel, options) {
    ...
    parentOp.getResponses().forEach(function(res, ndx, arr) {
        ...
        var expected = processResponse(res, parentOp.method, parentPath.path, options)
        ...
        transactions.push({
            'testLevelDescription': util.format('should respond %s for "%s"', res.statusCode, replaceNewlines(res.description)),
            ...
            'expected': expected,
            'hasValue': hasValue,
            'headers': processHeaders(res.statusCode, parentOp, parentPath, topLevel, options)
        })
    })
...
function processParams(code, op, path, options) {
    ...
    op.getParameters().forEach(function(param, ndx, arr) {
        var customValue = lookupCustomValue(param.name, param.in, code, op.method, path.path, options)

        if (param.in === 'body') {
            params.body = customValue !== undefined ? customValue : param.getSample()
        } else if (param.in === 'query') {
            params.query[param.name] = customValue !== undefined ? customValue : param.getSample()
        } else if (param.in === 'formData') {
            try {
                params.formData[param.name] = customValue !== undefined ? customValue : param.getSample()
...
function processHeaders(responseCode, op, path, top, options) {
    ...
    if (options.customValues) {
        if (options.customValues.headers) {
            headers = merge2(options.customValues.headers, headers)
        }

        if (options.customValues[path.path]) {
            if (options.customValues[path.path].headers) {
                headers = merge2(options.customValues[path.path].headers, headers)
            }

            if (options.customValues[path.path][op.method]) {
                if (options.customValues[path.path][op.method].headers) {
                    headers = merge2(options.customValues[path.path][op.method].headers, headers)
                }

                if (options.customValues[path.path][op.method][responseCode] && 
                    options.customValues[path.path][op.method][responseCode].headers) {
                    
                    headers = merge2(options.customValues[path.path][op.method][responseCode].headers, headers)
                }
            }
        }
    }
    ...
    if (options.consumes) { // a specific content-type was targeted
        ...
        } else if (top.consumes.length > 0 && top.consumes.indexOf(options.consumes) != -1) {
            headers['Content-Type'] = options.consumes;
    ...
    if (options.produces) {
        ...
        } else if (top.produces.length > 0 && top.produces.indexOf(options.produces) != -1) {
            headers['Accept'] = options.produces;
