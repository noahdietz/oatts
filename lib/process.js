...
function processTransactions(api, parentOp, parentPath, topLevel, options) {
    ...
    parentOp.getResponses().forEach(function(res, ndx, arr) {
        if (options.statusCodes && options.statusCodes.indexOf(res.statusCode) === -1) {
        ...
        transactions.push({
            ...
            'expected': expected,
            'hasValue': hasValue,
            'headers': merge2(params.headers, processHeaders(res.statusCode, parentOp, parentPath, topLevel, options))
        })
    })
...
function processParams(code, op, path, options) {
    ...
    op.getParameters().forEach(function(param, ndx, arr) {
        ...
        if (param.in === 'body') {
            ...
            params.headers[param.name] = customValue !== undefined ? customValue : param.getSample()
        }
    })
